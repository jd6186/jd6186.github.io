...
3. 2단계를 완료하셨으면 <strong style="color: #bb4177;">'작업 정의(Task)'</strong>를 생성해주시면 됩니다. 마찬가지로 Fargate를 선택해주시고 작업과 관련된 설정을 진행해주시면 됩니다.<br/>여기서 설정을 하긴 하지만 추후 배포 시 taskdefinition.json을 통해 설정을 변경할 수 있으니 가볍게 작업해주시면 됩니다.
   ...
   <strong style="color: #bb4177;">'taskdefinition.json'</strong> 샘플코드
   ```json
       {
...
5. 이제 제작된 작업 정의(Task)를 기반으로 클러스터 내 <strong style="color: #bb4177;">'Service'</strong>를 제작해줍니다.
   ...
   <strong style="color: #bb4177;">'ECS Blue/Green'</strong> 사용입니다.<br/>
   ...
   > 네트워크 구성 샘플<br/>
   > - 네트워크 구성 시 로드벨런서에 타겟 그룹 추가 필요<br/>
   ...
   VPC, Subnet, 보안그룹, 로드벨런서, IAM Role 등에 대한 설정은 보통 사내 Infra팀에서 모두 잡아주실 겁니다. 
...
3. <strong style="color: #bb4177;">'Build 스테이지'</strong> 생성 방법
   > Build 스테이지를 생성 샘플<br/>
   > <img src="../../../assets/img/2022-07-20-AWS_ECS_Fargate_Architecture/code_build.png"  width="700"/><br/>

   먼저 아래와 같이 세팅한 후 Code Build 프로젝트를 생성해주어야 합니다.<br/>(이 곳에서 프로젝트를 생성하셔야 프로젝트 생성 시 자동으로 소스공급자가 CodePipeline으로 설정됩니다.)

   <br/>
   
   > <strong style="color: #bb4177;">'Code Build 프로젝트'</strong> 생성 샘플<br/>
   > <img src="../../../assets/img/2022-07-20-AWS_ECS_Fargate_Architecture/code_build_project.png"  width="700"/>

   - 아래는 위 이미지에서 보신 <strong style="color: #bb4177;">'dev.buildspec.yml'</strong> 샘플 코드 
      ```yml
          version: 0.1
          ...
          artifacts:
              ...
              discard-paths: yes
      ```
      개발환경 예시라 dev가 앞에 붙은 것을 확인하실 수 있습니다.<br/>보시게 되면 $AWS_ACCOUNT_ID와 같이 환경변수를 가져온 것을 보실 수 있는데 이는 위에서 Code Build 프로젝트를 생성하실 때 지정하신 환경변수 내 값을 가져오는 것이므로 위에서 잘 세팅해주셔야 합니다.
...
4. <strong style="color: #bb4177;">'Code Deploy 스테이지'</strong> 생성 방법
   > Deploy 스테이지를 생성 샘플<br/>
   > <img src="../../../assets/img/2022-07-20-AWS_ECS_Fargate_Architecture/code_deploy_stage.png"  width="700"/>
   
   먼저 아래와 같이 세팅한 후 Code Deploy Application 및 Deploy Group을 생성해주어야 합니다.

   <br/>
   
   > <strong style="color: #bb4177;">'Code Deploy Application'</strong> 생성 샘플
   > <img src="../../../assets/img/2022-07-20-AWS_ECS_Fargate_Architecture/code_deploy_application.png"  width="700"/><br/>

   <br/>

   > <strong style="color: #bb4177;">'Deploy Group'</strong> 생성 샘플
   ...
   서비스 생성 시 지정한 역할(Role), 로드벨런서, 대상그룹(Target Group) ...<br/>

   추가 주의사항으로 원래 개정 종료 시간을 <strong style="color: #bb4177;">'0일 0시간 0분'</strong>으로 하지 않으면 ECS 서비스 내 새로운 작업이 생성되어도 저 시간동안 교체되지 않고 있게 됩니다. 그 때문에 저는 보통 0, 0, 0으로 세팅하고 있습니다.<br/>
   
   <br/><br/>
   
   - 위 이미지 내용 중 <strong style="color: #bb4177;">'dev.appspec.yml'</strong> 샘플<br/>
      ```yml
         version: 0.1
         Resources:
             - TargetService:
...
## 글을 마치며
오늘은 ECS와 CodePipeline을 연계한 CI/CD 환경 구축 파트를 알아보았습니다.
물론 위 내용만으로 서버를 오픈하기에는 여러 어려움이 존재합니다. 네트워크 요소들, 로그 작성, 배치 등등 대한 설명이 추가로 필요한 것은 알지만 
위 글을 통해 ECS를 어떻게 생성해야 하고 이를 CodePipeline과 연계하는 방법에 대해서는 충분히 설명을 드렸다고 생각됩니다.

추후 다른 글을 통해 네트워크 요소들(VPC, Subnet, Load Balancer, Secret Group ...)에 대한 내용을 다뤄보도록 하겠습니다.
