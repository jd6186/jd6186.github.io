...
데이터베이스 연산이 여러 작업으로 구성되어 있을 때, 전체 작업이 성공해야만 변경 사항을 반영하고, 실패 시에는 이전 상태로 복구(rollback)하여 데이터 일관성을 유지하는 중요한 역할을 합니다.

Spring Boot는 트랜잭션을 쉽게 관리할 수 있는 `@Transactional` 애너테이션을 제공합니다.
